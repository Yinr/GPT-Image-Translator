...
import type { FetchLike, HttpTransport } from "../src/adapters/http-transport.ts";
...
Deno.test("OpenAIAdapter sends image edit request and parses response", async () => {
  ...
  const client = new OpenAIAdapter(
    config,
    "test-key",
    testTransport(async (input, init) => {
...
Deno.test("OpenAIAdapter sends User-Agent when configured", async () => {
  ...
  const client = new OpenAIAdapter(
    ...
    "test-key",
    testTransport(async (_input, init) => {
      const requestInit = init as RequestInit | undefined;
...
function testTransport(fetch: FetchLike): HttpTransport {
  return { fetch, close: () => {} };
}
